...
# # from app.util.config import build_config

# # from app.util.db import request_sql
# # from app.util.db import get_sql_list
# from . import mail
...
#         df = process_report.prepare_df(df)  # подготовка датафрема
#         self.update_date(theme, subject)  # обновление даты
#         self.update_clients(df)  # добавление новых клиентов в бд
#         self.update_products(df)  # добавление новых продуктов в бд
#         self.update_sales(df)  # обновление продаж в бд


# def update_db():
#     cfg_path = "./data/config.json"  # путь к конфигу, если другой — измени
#     theme = "Продажи"  # тема письма — тоже можно параметризовать

#     updater = Updater(cfg_path)
#     updater.start()
#     updater.update_all(theme)
from .updater import *  # noqa